...
public class Drawer1 : MonoBehaviour
{
    ...
    [SerializeField] private Canvas canvas; // Canvas for interaction prompt (T for this drawer)
    ...
    void Update()
    {
        if(!WardrobeDoorController.doorsOpen) return; // can't open drawer when door is closed
